...
func TestValidateClaimMappings(t *testing.T) {
 ...
 | ^`,
		},
		{
			...
			structuredAuthnFeatureEnabled: false,
			want:                          `issuer.claimMappings.username.expression: Invalid value: "foo.bar": expression is not supported when StructuredAuthenticationConfiguration feature gate is disabled`,
		},
		{
			...
			structuredAuthnFeatureEnabled: false,
			want:                          `issuer.claimMappings.groups.expression: Invalid value: "foo.bar": expression is not supported when StructuredAuthenticationConfiguration feature gate is disabled`,
		},
		{
		...
		},
		{
			...
			in: api.ClaimMappings{
				Extra: []api.ExtraMapping{
					{Key: "key", ValueExpression: "claims.extra"},
				},
			},
			structuredAuthnFeatureEnabled: false,
		...
		},
		{
			...
			in: api.ClaimMappings{
				Username: api.PrefixedClaimOrExpression{Expression: pointer.String("claims.username")},
				Groups:   api.PrefixedClaimOrExpression{Expression: pointer.String("claims.groups")},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCELMapper {
				if tt.in.Username.Expression != nil && celMapper.Username == nil {
					t.Fatalf("ClaimMappings validation mismatch: CELMapper.Username is nil")
				}
				if tt.in.Groups.Expression != nil && celMapper.Groups == nil {
					t.Fatalf("ClaimMappings validation mismatch: CELMapper.Groups is nil")
				}
				if tt.in.UID.Expression != "" && celMapper.UID == nil {
