...
// TestAdmitExcludedVirtualResources verifies that MutatingAdmissionWebhook skips dispatch
// for resources in the injected excluded set only when the
// ExcludeAdmissionWebhookVirtualResources feature gate is enabled.
func TestAdmitExcludedVirtualResources(t *testing.T) {
	...
	disallowWebhook := []registrationv1.MutatingWebhook{{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ns := "webhook-test"
			client, informer := webhooktesting.NewFakeMutatingDataSource(ns, disallowWebhook, stopCh)
