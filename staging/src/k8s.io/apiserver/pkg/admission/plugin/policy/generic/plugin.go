...
func (c *Plugin[H]) ValidateInitialization() error {
	...
	if c.source == nil {
		...
		if len(c.staticManifestsDir) > 0 {
			...
			klog.InfoS("loaded static policy manifests", "dir", c.staticManifestsDir)
	...
	if c.source == nil {
		return fmt.Errorf("policy source is not properly configured")
	}
	if c.dispatcher == nil {
		return fmt.Errorf("policy dispatcher is not properly configured")
	}
	return nil
}
