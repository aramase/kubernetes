...
func (a *Plugin) SetManifestLoaders(loaders *initializer.ManifestLoaders) {
	...
	a.SetStaticSourceFactory(func(manifestsDir string) (generic.Source[PolicyHook], func(ctx context.Context) error, error) {
		staticSource := source.NewStaticPolicySource[*v1.MutatingAdmissionPolicy, *v1.MutatingAdmissionPolicyBinding, PolicyEvaluator](manifestsDir, a.GetAPIServerID(), compilePolicy,
...
func NewPlugin(configFile io.Reader) (*Plugin, error) {
	...
	if len(cfg.StaticManifestsDir) > 0 {
		klog.InfoS("mutating admission policy static manifests directory configured", "dir", cfg.StaticManifestsDir)
	}
