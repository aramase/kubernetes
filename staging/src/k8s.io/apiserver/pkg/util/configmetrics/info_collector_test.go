...
func TestAtomicHashProvider(t *testing.T) {
	provider := NewAtomicHashProvider("apiserver_id_hash", "hash")
	...
	// Test setting hashes
	provider.SetHashes(map[string]string{
	...
	provider.SetHash("hash", newConfigHash)
	hashes = provider.GetCurrentHashes()
...
func TestAtomicHashProviderPanic(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Errorf("Expected panic when creating AtomicHashProvider with no label names")
		} else {
			expectedMsg := "NewAtomicHashProvider: at least one label name is required"
			if r != expectedMsg {
