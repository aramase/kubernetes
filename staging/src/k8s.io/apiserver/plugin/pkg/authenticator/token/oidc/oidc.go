...
func New(opts Options) (*Authenticator, error) {
	url, err := url.Parse(opts.JWTAuthenticator.Issuer.URL)
	if err != nil {
		return nil, err
	}

	if url.Scheme != "https" {
		return nil, fmt.Errorf("'oidc-issuer-url' (%q) has invalid scheme (%q), require 'https'", opts.JWTAuthenticator.Issuer.URL, url.Scheme)
	}

	if opts.JWTAuthenticator.ClaimMappings.Username.Claim == "" {
	...
	verifierConfig := &oidc.Config{
		ClientID:             opts.JWTAuthenticator.Issuer.ClientIDs[0],
...
func (a *Authenticator) AuthenticateToken(ctx context.Context, token string) (*authenticator.Response, bool, error) {
	...
	if a.jwtAuthenticator.ClaimMappings.Username.Prefix != "" {
	...
	if a.jwtAuthenticator.ClaimMappings.Groups.Prefix != "" {
