...
func TestToken(t *testing.T) {
	...
	tests := []claimsTest{
		...
		},
		{
			...
			wantErr: `oidc: user validation rule failed: username cannot used reserved system: prefix`,
		},
		{
			...
			wantErr: `oidc: user validation rule failed: groups cannot used reserved system: prefix`,
		},
		{
			...
			options: Options{
				JWTAuthenticator: apiserver.JWTAuthenticator{
					...
					ClaimMappings: apiserver.ClaimMappings{
						...
						Groups: apiserver.PrefixedClaimOrExpression{
							Expression: pointer.String(`(claims.roles.split(",") + claims.other_roles.split(",")).map(role, "groups:" + role)`),
						},
					},
				},
				now: func() time.Time { return now },
