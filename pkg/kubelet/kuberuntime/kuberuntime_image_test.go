...
func TestPullImage(t *testing.T) {
	...
	imageRef, err := fakeManager.PullImage(ctx, kubecontainer.ImageSpec{Image: "busybox"}, nil, nil, nil, nil)
	assert.NoError(t, err)
...
func TestPullImageWithError(t *testing.T) {
	...
	imageRef, err := fakeManager.PullImage(ctx, kubecontainer.ImageSpec{Image: ":invalid"}, nil, nil, nil, nil)
	assert.Error(t, err)
...
func TestPullImageWithInvalidImageName(t *testing.T) {
	...
	for _, val := range imageList {
		ctx := context.Background()
		imageRef, err := fakeManager.PullImage(ctx, kubecontainer.ImageSpec{Image: val}, nil, nil, nil, nil)
		assert.Error(t, err)
		assert.Equal(t, "", imageRef)
...
func TestRemoveImage(t *testing.T) {
	...
	_, err = fakeManager.PullImage(ctx, kubecontainer.ImageSpec{Image: "busybox"}, nil, nil, nil, nil)
	assert.NoError(t, err)
...
func TestPullWithSecrets(t *testing.T) {
	...
	for description, test := range tests {
		...
		_, err = fakeManager.PullImage(ctx, kubecontainer.ImageSpec{Image: test.imageName}, test.passedSecrets, nil, nil, nil)
		require.NoError(t, err)
...
func TestPullWithSecretsWithError(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			imageRef, err := fakeManager.PullImage(ctx, kubecontainer.ImageSpec{Image: test.imageName}, test.passedSecrets, nil, nil, nil)
			assert.Error(t, err)
			assert.Equal(t, "", imageRef)
...
func TestPullThenListWithAnnotations(t *testing.T) {
	...
	_, err = fakeManager.PullImage(ctx, imageSpec, nil, nil, nil, nil)
	assert.NoError(t, err)
