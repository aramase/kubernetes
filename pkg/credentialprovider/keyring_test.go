...
func TestDockerKeyringForGlob(t *testing.T) {
   ...
   "%s": {
     ...
     "auth": %q
		...
		creds, ok := keyring.Lookup(test.targetURL+"/foo/bar", nil, nil)
		if !ok {
...
func TestKeyringMiss(t *testing.T) {
   ...
   "%s": {
     ...
     "auth": %q
		...
		_, ok := keyring.Lookup(test.lookupURL+"/foo/bar", nil, nil)
		if ok {
...
func TestKeyringMissWithDockerHubCredentials(t *testing.T) {
   ...
   "https://%s": {
	...
	val, ok := keyring.Lookup("world.mesos.org/foo/bar", nil, nil)
	if ok {
...
func TestKeyringHitWithUnqualifiedDockerHub(t *testing.T) {
   ...
   "https://%s": {
	...
	creds, ok := keyring.Lookup("google/docker-registry", nil, nil)
	if !ok {
...
func TestKeyringHitWithUnqualifiedLibraryDockerHub(t *testing.T) {
   ...
   "https://%s": {
	...
	creds, ok := keyring.Lookup("jenkins", nil, nil)
	if !ok {
		t.Errorf("Didn't find expected URL: %s", url)
		return
	}
...
func TestKeyringHitWithQualifiedDockerHub(t *testing.T) {
   ...
   "https://%s": {
	...
	creds, ok := keyring.Lookup(url+"/google/docker-registry", nil, nil)
	if !ok {
		t.Errorf("Didn't find expected URL: %s", url)
		return
	}
...
func TestDockerKeyringLookup(t *testing.T) {
	...
	for i, tt := range tests {
		match, ok := dk.Lookup(tt.image, nil, nil)
		if tt.ok != ok {
