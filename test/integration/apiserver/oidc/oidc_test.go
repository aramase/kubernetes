...
type authenticationConfigFunc func(t testing.TB, issuerURL, caCert string) string
...
func runTests(t *testing.T, useAuthenticationConfig bool) {
	var tests = []singleTest[*rsa.PrivateKey, *rsa.PublicKey]{
		{
			name: "ID token is ok",
			configureInfrastructure: func(t testing.TB, fn authenticationConfigFunc, keyFunc func(t testing.TB) (*rsa.PrivateKey, *rsa.PublicKey)) (
...
`, oidcServer.URL(), defaultOIDCClientID, indentCertificateAuthority(string(caCertContent)), defaultOIDCUsernamePrefix)
			...
			}, configureOIDCServerBehaviour: func(t testing.TB, oidcServer *utilsoidc.TestServer, signingPrivateKey *rsa.PrivateKey) {
			...
			assertErrFn: func(t testing.TB, errorToCheck error) {
		...
		},
		{
			...
			configureInfrastructure: configureTestInfrastructure[*rsa.PrivateKey, *rsa.PublicKey],
			configureOIDCServerBehaviour: func(t testing.TB, oidcServer *utilsoidc.TestServer, _ *rsa.PrivateKey) {
		...
		},
		{
			...
			configureInfrastructure:      configureTestInfrastructure[*rsa.PrivateKey, *rsa.PublicKey],
			configureOIDCServerBehaviour: func(t testing.TB, oidcServer *utilsoidc.TestServer, _ *rsa.PrivateKey) {},
			configureClient: func(t testing.TB, restCfg *rest.Config, caCert []byte, _, oidcServerURL, oidcServerTokenURL string) kubernetes.Interface {
				tempDir := t.TempDir()
	...
	for _, tt := range []singleTest[*ecdsa.PrivateKey, *ecdsa.PublicKey]{
		{
			...
			configureOIDCServerBehaviour: func(t testing.TB, oidcServer *utilsoidc.TestServer, signingPrivateKey *ecdsa.PrivateKey) {
				idTokenLifetime := time.Second * 1200
				oidcServer.TokenHandler().EXPECT().Token().Times(1).DoAndReturn(utilsoidc.TokenHandlerBehaviorReturningPredefinedJWT(
					t,
					signingPrivateKey,
...
type singleTest[K utilsoidc.JosePrivateKey, L utilsoidc.JosePublicKey] struct {
	name                    string
	configureInfrastructure func(t testing.TB, fn authenticationConfigFunc, keyFunc func(t testing.TB) (K, L)) (
		oidcServer *utilsoidc.TestServer,
		apiServer *kubeapiserverapptesting.TestServer,
		signingPrivateKey K,
		caCertContent []byte,
	...
	configureOIDCServerBehaviour func(t testing.TB, oidcServer *utilsoidc.TestServer, signingPrivateKey K)
...
func singleTestRunner[K utilsoidc.JosePrivateKey, L utilsoidc.JosePublicKey](
	...
	keyFunc func(t testing.TB) (K, L),
	tt singleTest[K, L],
) func(t *testing.T) {
	return func(t *testing.T) {
		fn := func(t testing.TB, issuerURL, caCert string) string { return "" }
		if useAuthenticationConfig {
			fn = func(t testing.TB, issuerURL, caCert string) string {
				return fmt.Sprintf(`
apiVersion: apiserver.config.k8s.io/v1alpha1
kind: AuthenticationConfiguration
jwt:
...
func TestUpdatingRefreshTokenInCaseOfExpiredIDToken(t *testing.T) {
	type testRun[K utilsoidc.JosePrivateKey] struct {
		name                            string
		configureUpdatingTokenBehaviour func(t testing.TB, oidcServer *utilsoidc.TestServer, signingPrivateKey K)
		assertErrFn                     func(t testing.TB, errorToCheck error)
	}
	...
	var tests = []testRun[*rsa.PrivateKey]{
		{
			...
			configureUpdatingTokenBehaviour: func(t testing.TB, oidcServer *utilsoidc.TestServer, signingPrivateKey *rsa.PrivateKey) {
				oidcServer.TokenHandler().EXPECT().Token().Times(1).DoAndReturn(utilsoidc.TokenHandlerBehaviorReturningPredefinedJWT(
					t,
					signingPrivateKey,
					map[string]interface{}{
	...
	oidcServer, apiServer, signingPrivateKey, caCert, certPath := configureTestInfrastructure(t, func(t testing.TB, _, _ string) string { return "" }, rsaGenerateKey)
	...
	tests := []testRun[*rsa.PrivateKey, *rsa.PublicKey]{
		{
			...
			authConfigFn: func(t testing.TB, issuerURL, caCert string) string {
				return fmt.Sprintf(`
apiVersion: apiserver.config.k8s.io/v1alpha1
kind: AuthenticationConfiguration
jwt:
...
func rsaGenerateKey(t testing.TB) (*rsa.PrivateKey, *rsa.PublicKey) {
	t.Helper()
...
func ecdsaGenerateKey(t testing.TB) (*ecdsa.PrivateKey, *ecdsa.PublicKey) {
	t.Helper()
...
func configureTestInfrastructure[K utilsoidc.JosePrivateKey, L utilsoidc.JosePublicKey](t testing.TB, fn authenticationConfigFunc, keyFunc func(t testing.TB) (K, L)) (
	oidcServer *utilsoidc.TestServer,
	apiServer *kubeapiserverapptesting.TestServer,
	signingPrivateKey K,
	caCertContent []byte,
...
func configureClientFetchingOIDCCredentials(t testing.TB, restCfg *rest.Config, caCert []byte, certPath, oidcServerURL, oidcServerTokenURL string) kubernetes.Interface {
...
func configureClientWithEmptyIDToken(t testing.TB, restCfg *rest.Config, _ []byte, certPath, oidcServerURL, _ string) kubernetes.Interface {
...
func configureRBAC(t testing.TB, clientset kubernetes.Interface, role *rbacv1.Role, binding *rbacv1.RoleBinding) {
	t.Helper()
...
func configureClientConfigForOIDC(t testing.TB, config *rest.Config, clientID, caFilePath, idToken, refreshToken, oidcServerURL string) *rest.Config {
	t.Helper()
...
func startTestAPIServerForOIDC[L utilsoidc.JosePublicKey](t testing.TB, c apiServerOIDCConfig, publicKey L) *kubeapiserverapptesting.TestServer {
	t.Helper()
...
func fetchOIDCCredentials(t testing.TB, oidcTokenURL string, caCertContent []byte) (idToken, refreshToken string) {
	t.Helper()
...
func fetchExpiredToken(t testing.TB, oidcServer *utilsoidc.TestServer, caCertContent []byte, signingPrivateKey *rsa.PrivateKey) (expiredToken, stubRefreshToken string) {
	t.Helper()
...
func configureOIDCServerToReturnExpiredIDToken(t testing.TB, returningExpiredTokenTimes int, oidcServer *utilsoidc.TestServer, signingPrivateKey *rsa.PrivateKey) {
	t.Helper()
...
func generateCert(t testing.TB) (cert, key []byte, certFilePath, keyFilePath string) {
	t.Helper()
...
func BenchmarkNoCELExpression(b *testing.B) {
	klog.SetOutput(io.Discard)
	klog.LogToStderr(false)

	defer featuregatetesting.SetFeatureGateDuringTest(b, utilfeature.DefaultFeatureGate, features.StructuredAuthenticationConfiguration, true)()

	caCertContent, _, caFilePath, caKeyFilePath := generateCert(b)
	signingPrivateKey, publicKey := rsaGenerateKey(b)
	oidcServer := utilsoidc.BuildAndRunTestServer(b, caFilePath, caKeyFilePath, "")
...
`, oidcServer.URL(), defaultOIDCClientID, indentCertificateAuthority(string(caCertContent)), defaultOIDCUsernamePrefix)
	...
	oidcServer.JwksHandler().EXPECT().KeySet().AnyTimes().DoAndReturn(utilsoidc.DefaultJwksHandlerBehavior(b, publicKey))
	apiServer := startTestAPIServerForOIDC(b, apiServerOIDCConfig{authenticationConfigYAML: authConfig}, publicKey)

	tokenURL, err := oidcServer.TokenURL()
	require.NoError(b, err)
	client := configureClientFetchingOIDCCredentials(b, apiServer.ClientConfig, caCertContent, caFilePath, oidcServer.URL(), tokenURL)
